...
export default function LinearAlgebraPlayground({ mode }: { mode: Mode }) {
  ...
  return (
    <section aria-label={modeTitle[mode]} className="mt-3 mb-7 rounded-lg border border-slate-200 bg-slate-50 p-3 sm:p-4">
      ...
      <p className="mt-3 rounded-md bg-white px-3 py-2 text-sm font-bold leading-6 text-slate-700" aria-live="polite">
        현재 입력은 [{values.vector.join(", ")}]이고, 스칼라 {scale}을 곱하면 [{values.scaled.join(", ")}]가 됩니다.
        행렬-벡터 곱 결과는 [{values.matrixVector.join(", ")}]입니다.
      </p>
