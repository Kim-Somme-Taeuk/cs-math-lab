...
export default function RelationPlayground() {
  ...
  const analysis = useMemo(() => {
    ...
    const missingSymmetric = pairs
      .map((item) => item.replace(/[()]/g, "").split(","))
      .map(([a, b]) => pair(b, a))
      .filter((reverse) => !pairs.includes(reverse));
    ...
    for (const first of pairs) {
      ...
      for (const second of pairs) {
        ...
        if (b === c && !pairs.includes(needed)) {
          missingTransitive.push(`${formatPair(first)}와 ${formatPair(second)}는 있지만 ${formatPair(needed)}가 없습니다.`);
        }
      }
    }
    ...
    return {
      ...
      reflexiveReason:
        missingReflexive.length === 0
          ? "모든 자기 자신 쌍이 있습니다."
          : `${missingReflexive.map(formatPair).join(", ")}가 없습니다.`,
      symmetricReason:
        missingSymmetric.length === 0
          ? "모든 쌍에 대해 반대 방향 쌍이 있습니다."
          : `${formatPair(missingSymmetric[0])}가 없습니다.`,
      transitiveReason:
        missingTransitive.length === 0 ? "이어지는 두 쌍이 있을 때 건너뛴 쌍도 있습니다." : missingTransitive[0],
    };
  }, [pairs]);
  ...
  return (
    <section aria-label="관계 성질 실험" className="mt-3 mb-7 rounded-lg border border-slate-200 bg-slate-50 p-3 sm:p-4">
      ...
      <p className="mt-1 text-sm text-slate-600">
        3x3 관계쌍 체크보드에서 순서쌍을 직접 고르고 반사성, 대칭성, 추이성을 확인합니다.
      </p>

      <div className="mt-4 grid gap-4 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <div className="h-[312px] rounded-lg border border-slate-200 bg-white p-3">
          <p className="text-sm font-bold text-slate-500">관계 R</p>
          <div className="mt-2 grid grid-cols-[2rem_repeat(3,minmax(0,1fr))] gap-1.5">
            <span />
            {elements.map((b) => (
          ...
          <p className="mt-3 overflow-x-auto whitespace-nowrap rounded-md bg-slate-50 p-2 text-sm leading-6 text-slate-700">
            R = {"{"} {pairs.map(formatPair).join(", ")} {"}"}
          </p>
        </div>
